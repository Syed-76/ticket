...
5. Copy `.env.example` to `.env` and fill in `DISCORD_TOKEN` and `CLIENT_ID`. `GUILD_ID` is optional: use a real server ID for instant test-guild commands, or leave it unset/a placeholder for automatic public global command registration.
